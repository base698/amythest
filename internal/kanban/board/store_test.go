...
func TestCreateBoardCreatesNoteBackedBoardAndRejectsDuplicateOrInvalidNames(t *testing.T) {
	...
	if created.Name != "new-project" || created.Version != 4 || !created.Pinned || created.DispatchEnabled || len(created.Cards) != 0 {
	...
	for _, name := range []string{"board.md", "done.md"} {
		...
		if !strings.Contains(string(payload), "kanban: 4") || !strings.Contains(string(payload), "name: new-project") {
...
func TestRenderBoardExposesStableCardIDBlockLink(t *testing.T) {
	...
	if !strings.Contains(string(content), "^"+card.ID) {
...
func TestStoreCreatesReadableBoardAndArchivesDoneCard(t *testing.T) {
	...
	text := string(boardMD)
	for _, want := range []string{"name: proof", "# Ready", "## Deploy Proof production with login", "assignee:: Operator"} {
...
func TestBoardMetadataCanBeCreatedAndPatchedWithoutErasingOmittedFields(t *testing.T) {
	...
	if created.Version != 4 || created.DisplayName != "Research Lab" || !created.Pinned || created.DispatchEnabled {
		t.Fatalf("created board = %#v", created)
	}
	nextName := "Research"
	archived := true
